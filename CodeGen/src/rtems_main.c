...
static volatile int stop_model = 0;
static double T = 0.0;
static double Tsamp;
...
static rtems_task rt_task(rtems_task_argument argument)
{
  struct pysim_platform_model_ctx *mctx = (struct pysim_platform_model_ctx*) argument;
...
rtems_task Init(rtems_task_argument ignored) {
  ...
  status = rtems_task_start( rt_task_id, rt_task, (rtems_task_argument)&NAME(MODEL, _pt_ctx) );
  if(!rtems_is_status_successful(status)) {
